...
vector<int> keys[1101]; // keys[i] : i번 돼지우리 키를 갖고 있는 손님 번호
int main(){
    ios::sync_with_stdio(0); cin.tie(0);
    int M, N; cin>>M>>N;
    MaxFlow MF(1111);
    int sNum=0, tNum=1101;
    for(int i=1; i<=M; ++i){
        int a; cin>>a;
        MF.addEdge(i+100,tNum,a);
    }
    for(int i=1; i<=N; ++i){
        int A; cin>>A;
        for(int j=0; j<A; ++j){
            int key; cin>>key;
            MF.addEdge(i,key+100,987654321);
            for(auto it:keys[key]){
                MF.addEdge(i, it, 987654321);
            }
